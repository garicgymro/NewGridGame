...
class Glbl():
    ...
    substitutionParam = {"max_unique":1, "num_substitutions":1, "min_interval":1,"threshold":0.75, "num_previous_instances":4}
            ...
            for indata in indata_list:
                ...
                # Also sends it to the speaker.
                    ...
                    #     print("full_message_dictionary.get(referent) before getting from dict", self.full_message_dictionary.get(self.referent))
                        ...
                        if referent in Glbl.full_message_dictionary[str(self.player.partner)]:
                            Glbl.full_message_dictionary[str(self.player.partner)][referent][-1]["correct"] = True
                        else:
                            Glbl.full_message_dictionary[str(self.player.partner)][referent]=[{"correct":True}]

                        Glbl.success_dict[referent] += 1
                ...
                # Maybe write a new class called metadata - sync server1 and server2
                    ...
                    OriginalFilledArr = ast.literal_eval(m.group(1))
                    ...
                    if Glbl.referent not in Glbl.complexityTracker.keys():
                        Glbl.complexityTracker[Glbl.referent] = {}
                        for player in self.gui.playerlist:
                            Glbl.complexityTracker[Glbl.referent][str(player)] = 0
                    ...
                    if Glbl.referent in Glbl.full_message_dictionary[str(self.player)]:
                        Glbl.full_message_dictionary[str(self.player)][Glbl.referent].append(single_message_dict)
                    else:
                        Glbl.full_message_dictionary[str(self.player)][Glbl.referent] = [single_message_dict]
                    ...
                    if float(Glbl.success_dict[Glbl.referent])/float(Glbl.substitutionParam["num_previous_instances"]) >= Glbl.substitutionParam["threshold"]:
                        threshold_met = True
                    else:
                        threshold_met = False
                    ...
                    if Glbl.conditions[Glbl.condition_index]["substitute"] == True:
                        print("this is a substitution condition")
                        if Glbl.substitutionCount < Glbl.substitutionParam["num_substitutions"]:
                    ...
                    if Glbl.referent not in Glbl.full_message_dictionary[str(self.player)]:
                        print("Not yet in dictionary")
                    if len(Glbl.full_message_dictionary[str(self.player)][Glbl.referent]) <2:
                        print("too short")

                    if (testMode == True 
                        or (Glbl.conditions[Glbl.condition_index]["substitute"] == True
                            and Glbl.referent in Glbl.full_message_dictionary[str(self.player)]
                            and len(Glbl.full_message_dictionary[str(self.player)][Glbl.referent]) > 1
                            and Glbl.substitutionCount < Glbl.substitutionParam["num_substitutions"]
                            and Glbl.roundsSinceLastSubstitution >= Glbl.substitutionParam["min_interval"]
                            and threshold_met == True)):
                            # and len(Glbl.complexityTracker.keys()) > Glbl.substitutionParam["max_unique"]
                            # and Glbl.referent in proportions[str(self.player)]
                            # and proportions[str(self.player)][Glbl.referent] >= Glbl.substitutionParam["threshold"])):
                        print("Substitution!!!")

                        Glbl.substitution_round=True
                        Glbl.roundsSinceLastSubstitution = 0
                        Glbl.substitutionCount += 1
                    ...
                    #     testMode = True
                    #     filledArr = self.complexitySubstitution(filledArr, Glbl.substitutionParam, testMode)
                    #     testMode = False
                    ...
                    if Glbl.substitution_round == True:
...
#Make sure to record everything here in results!!!

                    # Send coords and referent to the listener
                    receiver_message = "9999COORD" + ReceiverFilledStr + "@" + Glbl.referent + "9999"
                    sender_message = "9999COORD" + SenderFilledStr + "@" + Glbl.referent + "9999"
    ...
    def mostComplex(self,referent,player):
        ...
        targetList = Glbl.full_message_dictionary[player][referent]  
        # print(targetList)
    ...
    def complexitySubstitution(self, filledArr,player):
        ...
        Glbl.complexityTracker[Glbl.referent][str(player)] += 1


        # Glbl.complexityTracker[Glbl.referent][0] += 1
        # Glbl.complexityTracker[Glbl.referent][1] += 1
        print("SUBSTITUTED")
        return self.mostComplex(Glbl.referent,player)
    ...
    def noise(self,filledArr):
        print("noisy!")
        noiseThreshold = Glbl.conditions[Glbl.condition_index]["noise"][1]
        copy = filledArr
