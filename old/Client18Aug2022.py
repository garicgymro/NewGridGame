    ...
    def incoming_message(self, message):
        ...
        if re.match(self.srvr_msg, message):
            if re.match(self.kill_msg, message):
                ...
                Glbls.roundLength = int(m.group(4))
                Glbls.minRoundLength = int(m.group(5))
                Glbls.reduceRoundTime = ast.literal_eval(m.group(6))
                Glbls.roundTimeReduction = float(m.group(7))
                swap_param = m.group(8)
                if swap_param == "True":
                    self.swap = True
                else:
                    self.swap = False
