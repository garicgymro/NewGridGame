...
class GuiWindow(wx.Frame):
    ...
    def speaker_draw_stage(self):
        ...
        # if Glbl.display_all_referents_to_speaker == True:
    ...
    def display_pic_stage(self):
        ...
        if self.practice_over == False:
            if str(self.listener_pic_for_displaying) == str(self.correctReferent): #This doesn't mean anything ... !!!
                ...
                msg = "You picked the right referent!"
                print("previous score", self.score)
                self.score += 1
                print("practice score increased by 1!", self.score)
                print("**exit display_pic_stage**")
            else:
                msg = "You picked the wrong referent!"
        else:
            ...
            if str(self.listener_pic_for_displaying) == str(self.correctReferent):
                ...
                msg = "You picked the right referent!" # originally displayed running score
            else:
                msg = "You picked the wrong referent!" # originally displayed running score
    ...
    def incoming_message(self, message):
        ...
        if re.match(self.srvr_msg, message):
            if re.match(self.kill_msg, message):
                ...
                print("correct referent (sent from speaker):", m.group(2))

                # Extract the correct referent
                self.correctReferent = m.group(2)
                if self.role == "listener":
                    self.listener_choice_stage()
                else:
...
class ListenerButtonsPanel(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        self.box_sizer = wx.BoxSizer(wx.VERTICAL)
        # Issue here
        self.grid_sizer = wx.FlexGridSizer(6,2,5,5)
...
#Need to move referent list down and centered a bit. Also make sure submit button is centered relative to the grid
        ...
        if Glbl.display_all_referents_to_speaker == True:
            ...
            reflist = ""
        self.reftext = wx.StaticText(self, wx.ID_ANY, label=reflist)
        self.reftext.SetFont(wx.Font(12.0, wx.DEFAULT, wx.NORMAL,wx.NORMAL))
        ...
        self.referent_length = len(self.gui.referent_list)
        self.referent_index = random.randint(0, self.referent_length - 1)
        self.gui.referent_string = self.gui.referent_list[self.referent_index]
        self.referent_display = wx.StaticText(self, label= self.gui.referent_list[self.referent_index])
        ...
        self.hor_box_sizer.Add(self.reftext,0)
